class Jugador {
  constructor(x, y) {
    ...
    this.cooldown = 0; //tiempo de espera entre cada disparo
  }

//metodos
  actualizar() {
    // Movimiento horizontal
    if (this.moverIzq) this.x -= this.vel; //si moverizq es true le resta la velocidad a la pos en x
    if (this.moverDer) this.x += this.vel;  //si moverder es true le suma la velocidad a x

    // Movimiento vertical
    if (this.moverArriba) this.y -= this.vel;
    if (this.moverAbajo) this.y += this.vel;

    // Limites de pantalla (para que no se salga de canvas)
    this.x = constrain(this.x, 20, width - 20); //lo entendimos pero no lo dimos
